...
# Resolve governance repo (used in placeholder substitution + .exocortex.env).
# Priority: explicit environment override, default repository, then legacy/local names.
# When no repository exists yet, keep the default for the first seed ritual.
GOVERNANCE_REPO="${IWE_GOVERNANCE_REPO:-}"
DEFAULT_GOVERNANCE_REPO="${IWE_GOVERNANCE_REPO:-DS-strategy}"
if [ -z "$GOVERNANCE_REPO" ] && [ -d "$WORKSPACE_DIR/$DEFAULT_GOVERNANCE_REPO" ]; then
    GOVERNANCE_REPO="$DEFAULT_GOVERNANCE_REPO"
fi
...
GOVERNANCE_REPO="${GOVERNANCE_REPO:-$DEFAULT_GOVERNANCE_REPO}"
